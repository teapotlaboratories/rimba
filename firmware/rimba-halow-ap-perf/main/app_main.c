...
/* iperf throughput test (temporary): bring the AP up, skip the per-STA pinging, and start an
 * esp_console REPL so `iperf -s` can be run over serial (the STA is the client). Comment out
...
static void start_iperf_console(void)
{
    ...
    ESP_LOGW(TAG, "iperf console ready: `iperf -s` (server; STA is the client)");
}
#endif
