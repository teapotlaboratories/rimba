...
#include "test_report.h"   /* T2 mesh-ap: this is the far MESH PEER support role (up-marker) */
...
/* The gate is board2 (10.9.9.108) in this T2 rig, so the peer's return route to the AP subnet goes
 * via board2, not the product default board0 (10.9.9.136). Off-subnet echo replies to the STA
 * (192.168.12.x) route here or they die in lwIP ARP. */
#define MESH_GATE_IP    "10.9.9.108"
...
static void mesh_net_task(void *arg)
{
    ...
    /* Up-marker the orchestrator waits on (Role.up_marker="mesh-peer-up"). Support role: no RESULT. */
    TEST_INFO("mesh-peer-up: far node %s, return route to the AP subnet via gate %s", ipbuf,
                 MESH_GATE_IP);
...
void app_main(void)
{
    TEST_BEGIN("mesh-ap-peer", "far mesh node behind the gate (support role)");
