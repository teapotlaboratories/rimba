...
- **The ESP and Linux beacon paths are closer than this doc originally claimed — CORRECTED 2026-07-30 (S0b scoping).** The original text here said Linux FW-offloads the beacon + RPS while the ESP host-builds it, and put that asymmetry "exactly on the enforcement boundary." That is wrong: **enabling RAW switches Linux's beacon offload OFF.** `morse_hw_beacon_can_offload()` returns false when `morse_raw_is_enabled(mors_vif)` (`hw_beacon.c:398`, `:415-416`), and `WLAN_EID_S1G_RPS` is absent from the offload IE allowlist (`hw_beacon.c:17-24`). So with RAW on, **both** hosts build and serve the beacon every TBTT, and **neither sends the chip any RAW command** — the RPS element in the beacon is the firmware's only RAW input, on both platforms. The residual ESP-specific difference is narrower and sharper: Linux hands the chip a **fully-converted S1G** beacon (converted host-side in `dot11ah.ko`), whereas the ESP hands it a **legacy PV0** beacon that the FW converts for AP-type vifs (Map 3 §1, `umac_mesh.c:214-218`). That narrower difference is the whole of Q2 — and it creates a third outcome §5 now tracks: the element can be carried through to air by the converter while the RAW engine never arms, because the engine may read the host-supplied buffer rather than its own post-conversion output.
...
2. **Q2** — does a host-appended RPS IE survive the FW's legacy→S1G beacon re-conversion and reach air intact, byte-matching a live Linux RAW AP? (Map 3 §1).
...
- **[HIGH] Q2 third outcome: the element reaches air while nothing arms.** Not just strip-or-survive. Because the ESP hands the chip a **legacy PV0** beacon that the FW converts (whereas Linux hands it a fully-converted S1G one), the converter may carry the element through to air while the RAW engine — reading the host-supplied buffer rather than its own post-conversion output — never sees it. **Q2 PASS followed by a flat-counter Q3b would then be misread as "no AP-direction engine" and reported BLOCKED, when the cause is a host-format mismatch with a known in-tree fix** (the mesh pattern, `umac_mesh.c:210-218`). This is the most likely path to a false BLOCKED after the EID error. Never report a Q3b verdict from the capture alone — read the ESP's tag-4210 `assignments[]` in the same run.
- **[HIGH] FW legacy→S1G beacon re-conversion may strip/mangle the host RPS IE.** An appended RPS may not survive (Map 3 §1, `umac_mesh.c:214-218`). The mesh sidesteps this by emitting the whole S1G beacon host-side. **This is a COST question, not a gate** — both outcomes are implementation choices (§7.4).
- **[MED] The AP's S1G-caps RAW bit may never reach air.** Linux **deletes and regenerates** the S1G Capabilities element during conversion (`ie.c:518` + `tx_11n_to_s1g.c:922`), so the ESP FW plausibly does too — in which case flipping `s1g_capabilities.c:276-280` is invisible and **S3's "wire the advertise bit on when enabled" is dead as written**. Not listed in the original recon. Compounding it: on Linux the AP's own RAW bit is **coupled to RAW being enabled** (`mac.c:1392`), which is evidence the bit is load-bearing rather than decorative — so a Q3b negative caused by a stripped caps bit would be indistinguishable from "no engine." **S0b-3 settles it for one flash, no STA.**
...
| Spike-edit patch capture | **DONE.** The S0a delta (morselib accessors, manifest row, fixture tarball, doc edits) is captured as named patch files, each verified to apply in reverse, so the tree is restorable to the pre-S0b state deterministically. |
...
- **Deferred, tracked here so it is not re-derived:** `hapd->conf->raw_enabled` alone would activate
  two already-compiled hostapd features (QoS-Traffic-Cap priority learning `ieee802_11.c:5046`;
  priority-grouped AID allocation `:3948`) — most of S5 for near-zero code. **But it is blocked as
  written:** hostapd's groups are 256 AIDs wide up to 2007, while morselib caps AIDs at 255
  (`traffic_bitmap.h:22`) and at `max_stas` (default 20), so any STA with `raw_priority ≥ 1` gets
