...
- ☐ **Stack bump** (MM6108 fw / morselib / ESP-IDF) — a real port-forward, keep parity with the Linux node → ibss TODO #15. Also the **fork migration** to a real `MorseMicro/esp-halow` fork (history preserved): a first plan was drafted 2026-07-16 and **removed at the owner's request to restart from scratch** — to be re-planned, gated on the regression suite being green. Detail in [`mesh-ap/rimba-mesh-ap-milestones.md`](mesh-ap/rimba-mesh-ap-milestones.md) (Infra).
  - ☐ **Fork migration → `teapotlaboratories/esp-halow`** (owner-directed 2026-07-18, **now unblocked** — suite green, [#36](https://github.com/teapotlaboratories/rimba/pull/36) merged; **not started**). Goal: replay ALL our morselib work from `teapotlaboratories/mm-esp32-halow` (current `components/halow`) onto `teapotlaboratories/esp-halow` (the proper fork of the **official** `MorseMicro/esp-halow`), preserving commit history + PRs if possible, then repoint `components/halow`. **Investigation (verified with gh+git):** ours is **standalone** (isFork:false), base `68ddbd8d` = 2.10.4-esp32-2 import, 69 commits / 24 merged PRs above it; esp-halow is at **2.12**. **Three decisive findings:** (1) **disjoint histories** (no `merge-base`) → replay (cherry-pick / `format-patch`+`am -3`), not a shared-base rebase; (2) **version gap 2.10.4→2.12** = a forward-port, conflicts expected; (3) **possible layout mismatch** (ESP Component Registry pkg vs mm-iot-sdk tree) → **first step = diff `68ddbd8d` vs `esphalow/main`**. "Preserve history" = per-commit metadata survives but new SHAs on 2.12; "preserve PRs" = recreate (can't move across repos). After repointing, **re-run the WHOLE suite against 2.12**. Full handoff: worklog [`2026-07-18-fork-migration-esp-halow-handoff.md`](worklog/2026-07-18-fork-migration-esp-halow-handoff.md) + memory `fork-migration-esp-halow`.
