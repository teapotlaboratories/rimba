...
- ◐ **Mesh-gate (Mesh + AP)** → [`mesh-ap/rimba-mesh-ap-milestones.md`](mesh-ap/rimba-mesh-ap-milestones.md) (TODO section). Headline open items: **port 802.11s mesh into morselib** (the big one — blocks an all-ESP32 relay; **recon done**, phased P0–P5, next is the P0 firmware probe — see below), **RAW (Restricted Access Window)** AP-side port, **AID ≥ 64** on-air validation, Linux STA as TWT requester. *(Action-frame TWT now ✅ — [PR #15](https://github.com/teapotlaboratories/rimba/pull/15).)*
- ◐ **802.11s mesh port** → [`mesh-ap/rimba-mesh-ap-milestones.md`](mesh-ap/rimba-mesh-ap-milestones.md) (802.11s item) + recon [`worklog/2026-06-24-mesh-80211s-port-recon.md`](worklog/2026-06-24-mesh-80211s-port-recon.md). **P0 firmware probe ✅** (fw recognizes `ADD_INTERFACE(MESH=5)`). **Next: P1** — mesh vif up + beacon (Mesh ID/Config IEs + `MESH_CONFIG` command), oracle = Linux `iw mesh`.
