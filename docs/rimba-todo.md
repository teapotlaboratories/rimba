...
- ◐ **Mesh-gate (Mesh + AP)** → [`mesh-ap/rimba-mesh-ap-milestones.md`](mesh-ap/rimba-mesh-ap-milestones.md) (TODO section). The 802.11s mesh half is now **built (P0–P6b ✅)**; the remaining structural item is **mesh + AP concurrency on one ESP32 radio** (A3). Other open items: **RAW (Restricted Access Window)** AP-side port, **AID ≥ 64** on-air validation, Linux STA as TWT requester. *(Action-frame TWT now ✅ — [PR #15](https://github.com/teapotlaboratories/rimba/pull/15).)*
- ✅ **802.11s mesh port (P0–P6b)** → [`mesh-ap/rimba-mesh-ap-milestones.md`](mesh-ap/rimba-mesh-ap-milestones.md) (§802.11s mesh point) + worklog [`worklog/2026-06-26-mesh-mpm-peering-frames.md`](worklog/2026-06-26-mesh-mpm-peering-frames.md). An ESP32 joins a Linux HaLow mesh, pings, originates + **relays** multi-hop, group-forwards, and does **PERR** broken-link teardown. **Next (P6c):** rate-derived airtime metric, proxy/gate (`mpp`) for the Mesh-gate bridge, RANN/root, AMPE/SAE, mesh power save; plus an iperf throughput test + demo-scaffolding cleanup before merge.
