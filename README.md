...
| [`docs/mesh-ap/rimba-mesh-80211s-code-map.md`](docs/mesh-ap/rimba-mesh-80211s-code-map.md) | **802.11s mesh new-code ↔ Linux map.** Function-level table: each `umac_mesh_*`/datapath symbol ↔ its `net/mac80211`/`morse_driver` `file:line` equivalent (peering, HWMP, path table, forwarding, the 3 on-air fixes) + deliberate divergences. |
